...
impl<'a> TextAnim<'a> {
    ...
    pub fn advance_and_draw(&mut self, x: f32, y: f32, speed_ms: u32, d_box_line_tex: Texture2D) {
        ...
        draw_rectangle(32.0, screen_height()-200.0, screen_width()-64.0, 180.0, DARKGREEN);
        //DRAW STUFF AROUND DBOX
        draw_texture_ex(d_box_line_tex, 32., 388., WHITE, DrawTextureParams {
            dest_size: Option::from(Vec2 { x: screen_width()-64., y: 12. }),
            ..Default::default()
        });
        draw_texture_ex(d_box_line_tex, 32., screen_height()-20., WHITE, DrawTextureParams {
            dest_size: Option::from(Vec2 { x: screen_width()-64., y: 12. }),
            ..Default::default()
        });
        draw_texture_ex(d_box_line_tex, -76., 484., WHITE, DrawTextureParams {
            dest_size: Option::from(Vec2 { x: 204., y: 12. }),
            rotation: 1.5708,
            ..Default::default()
        });
        draw_texture_ex(d_box_line_tex, 672., 484., WHITE, DrawTextureParams {
            dest_size: Option::from(Vec2 { x: 204., y: 12. }),
            rotation: 1.5708,
            ..Default::default()
        });
